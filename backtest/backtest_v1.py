...
CONFIG: dict[str, Any] = {
    ...
    # 复权类型，支持 cq / qfq / hfq / dypre；默认使用不复权 cq 分析
...
class StrategyPriceDataFeed(bt.feeds.PandasData):
    lines = (
        "turn",
        "raw_open",
        "raw_high",
    ...
    params = (
        ...
        ("raw_low", "raw_low"),
        ("raw_close", "raw_close"),
        ("raw_preclose", "raw_preclose"),
        ("signal_factor", "signal_factor"),
        ("position_adjust_ratio", "position_adjust_ratio"),
    )


class DynamicPreBackBroker(bt.brokers.BackBroker):
    _DYPRE_EPSILON = 1e-12

    def _get_price_from_line(self, data, field: str) -> float:
        raw_line = getattr(data.lines, f"raw_{field}", None)
        if raw_line is None:
            return float(getattr(data, field)[0])
        raw_value = raw_line[0]
        if pd.isna(raw_value):
            return float(getattr(data, field)[0])
        return float(raw_value)

    def _apply_dynamic_pre_adjustments(self) -> None:
        for data, position in self.positions.items():
            setattr(data, "_dypre_applied_ratio", 1.0)
            ratio_line = getattr(data.lines, "position_adjust_ratio", None)
            if ratio_line is None or not position or not len(data):
                continue
            ratio = pd.to_numeric(pd.Series([ratio_line[0]]), errors="coerce").iloc[0]
            if pd.isna(ratio):
                continue
            ratio = float(ratio)
            if ratio <= 0 or abs(ratio - 1.0) <= self._DYPRE_EPSILON:
                continue
            current_price = position.price
            new_price = current_price / ratio if current_price else 0.0
            position.fix(position.size * ratio, new_price)
            position.adjbase = self._get_price_from_line(data, "close")
            setattr(data, "_dypre_applied_ratio", ratio)

    def _try_exec(self, order):
        data = order.data

        popen = self._get_price_from_line(data, "open")
        phigh = self._get_price_from_line(data, "high")
        plow = self._get_price_from_line(data, "low")
        pclose = self._get_price_from_line(data, "close")
        ...
        if order.exectype == order.Market:
            ...
            order.triggered
            and order.exectype in [order.StopLimit, order.StopTrailLimit]
        ):
    ...
    def _get_value(self, datas=None, lever=False):
        ...
        while self._cash_addition:
            cash_delta = self._cash_addition.popleft()
            self._fundshares += cash_delta / self._fundval
            self.cash += cash_delta

        for data in datas or self.positions:
            comminfo = self.getcommissioninfo(data)
            position = self.positions[data]
            mark_price = self._get_price_from_line(data, "close")
            if not self.p.shortcash:
                dvalue = comminfo.getvalue(position, mark_price)
            else:
                dvalue = comminfo.getvaluesize(position.size, mark_price)
        ...
        if not self._fundhist:
            self._value = pos_value_unlever + self.cash
    ...
    def next(self):
        self._apply_dynamic_pre_adjustments()
        super().next()
...
class SimpleMovingAverageStrategy(HStrategy):
    ...
    def _get_trade_price(self, field: str, ago: int = 0) -> float:
        ...
        if raw_line is None:
            return float(getattr(self.data, field)[ago])
        raw_value = raw_line[ago]
        if pd.isna(raw_value):
            return float(getattr(self.data, field)[ago])
        return float(raw_value)
    ...
    def _convert_trade_price_to_signal(self, trade_price: float, ago: int = 0) -> float:
        signal_factor_line = getattr(self.data, "signal_factor", None)
        if signal_factor_line is None:
            return trade_price
        factor_value = signal_factor_line[ago]
        if pd.isna(factor_value):
            return trade_price
        return float(trade_price) * float(factor_value)

    def _sync_dypre_state(self) -> None:
    ...
    def notify_order(self, order: bt.Order) -> None:
        ...
        if order.status == order.Completed:
            ...
            trade_price = float(order.executed.price)
            signal_price = self._convert_trade_price_to_signal(trade_price)
            turnover = executed_size * trade_price
            ...
            price_suffix = (
                f" | 信号价={signal_price:.2f}"
                if abs(signal_price - trade_price) > 1e-9
                else ""
            )
    ...
    def record_water_up_down_days(self) -> str:
        ...
        current_turn = getattr(self.data, "turn", None)
        if current_turn is not None and not pd.isna(current_turn[0]):
            area["turn_sum"] += float(current_turn[0])
        if is_up:
    ...
    def get_power(self, is_above=None) -> float:
        ...
        for i, area in enumerate(areas):
            ...
            if i >= 4:
                ...
                # 第3、4个权重
                weight = 0.6
            turnover_boost = 1 + min(float(area.get("turn_sum", 0.0)) / 100.0, 1.0)
            effective_weight = weight * turnover_boost
            _is_above = area["is_above_weight"]
            if _is_above:
                above_power += area["day_num_weight"] * effective_weight
            else:
                below_power += area["day_num_weight"] * effective_weight
        ...
        for i in range(l - 1, -1, -1):
            ...
            if i < l - 1:
                if prev_is_above != _is_above:
                    ...
                    areas[-1]['turn_sum'] += area.get('turn_sum', 0.0)
    ...
    def get_last_minmax_price(self, is_highest=True, weighted=False):
        # 权重计算方法：找到最近N个价，默认更偏向近期小趋势，避免长期大趋势把权重拉得过高
        if len(self.water_surface_areas) < 2:
            return None
        # 取最后N次最高价
        total_weight = 0.0
        ...
        for i in range(len(self.water_surface_areas) - 2, -1, -1):
            ...
            total_weight += max(float(area["day_num"]) ** 0.5, 1.0)
            if len(target_areas) >= need_num:
                break
        value = 0
        for area in target_areas:
            v = area["highest_price"] if is_highest else area["lowest_price"]
            area_weight = max(float(area["day_num"]) ** 0.5, 1.0)
            value += v * area_weight / total_weight
        return value

    def clear_last_lowest_price(self) -> None:
        # 触发止损时，表示上次买入时参照的最低价已经失真，要清掉，防止后续继续引用旧低点。
        for i in range(len(self.water_surface_areas) - 2, -1, -1):
            # 最后一个是当前价，不用遍历，所以是-2
            area = self.water_surface_areas[i]
            if area["is_above"]:
    ...
    def _check_and_sell(self) -> None:
        ...
        if self.last_buy_price is not None:
            ...
            if self.sell_trigger_active and current_close > self.last_buy_price:
                stop_loss_price = max(
                    stop_loss_price,
                    self.last_buy_price * (1 + float(self.param.get("protect_profit_floor_pct", 0.02))),
                )
        ...
        underwater_take_profit_pct = float(
            self.param.get("underwater_take_profit_pct", 0.05)
        )
        above_water_take_profit_pct = float(
            self.param.get("above_water_take_profit_pct", 0.20)
        )
        if current_close > self.last_buy_price * (1 + underwater_take_profit_pct):
            power = self.get_power()
            if not self.water_surface_areas[-1]['is_above']:
                if power > 0:
                    ...
                    self._active_sell_trigger(reason)
            elif current_close > self.last_buy_price * (1 + above_water_take_profit_pct):
                if power < 0.5:
                    reason = (
...
def build_data_feed(
    ...
    feed_df = df.copy()
    if "raw_open" not in feed_df.columns:
        feed_df["raw_open"] = feed_df["open"]
    if "raw_high" not in feed_df.columns:
        feed_df["raw_high"] = feed_df["high"]
    if "raw_low" not in feed_df.columns:
        feed_df["raw_low"] = feed_df["low"]
    if "raw_close" not in feed_df.columns:
        feed_df["raw_close"] = feed_df["close"]
    if "raw_preclose" not in feed_df.columns:
        feed_df["raw_preclose"] = feed_df["close"].shift(1)
    if "signal_factor" not in feed_df.columns:
        feed_df["signal_factor"] = 1.0
    if "position_adjust_ratio" not in feed_df.columns:
        feed_df["position_adjust_ratio"] = 1.0
    data_kwargs = {
        "dataname": feed_df,
    }
    if from_date:
    ...
    return StrategyPriceDataFeed(**data_kwargs)
...
def create_cerebro(config: dict[str, Any]) -> bt.Cerebro:
    ...
    broker = DynamicPreBackBroker()
    cerebro.broker = broker
    broker.setcash(config["cash"])
